    ...
    return (
      <div style={{ textAlign: "center", padding: "56px 24px" }} className="fade-in">
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <button type="button" onClick={start} style={{ padding: "16px 32px", borderRadius: 14, background: "var(--navy)", color: "white", fontWeight: 800, fontSize: 18, border: "none", cursor: "pointer" }}>Try Again</button>
          <button type="button" onClick={() => { setMode((m) => m === "arToEn" ? "enToAr" : "arToEn"); }} style={{ padding: "16px 32px", borderRadius: 14, background: "white", color: "var(--navy)", fontWeight: 800, fontSize: 18, border: "3px solid var(--navy)", cursor: "pointer" }}>Switch Mode</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 17, fontWeight: 700, color: "var(--navy)" }}>{current + 1}/{questions.length}</span>
      ...
      <div style={{ borderRadius: 22, background: "var(--navy)", padding: "36px 24px", textAlign: "center", marginBottom: 22 }}>
        ...
        {mode === "arToEn" ? (
          <>
            <div className="arabic" style={{ fontSize: "clamp(56px, 14vw, 88px)", color: "var(--gold-light)", lineHeight: 1.6, marginBottom: 16 }}>{word.arabic}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {options.map((opt) => {
          ...
          return (
            <button key={opt.id} type="button" onClick={() => handleAnswer(opt)} style={{
              ...
              {mode === "arToEn" ? opt.english : (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  ...
                  <span className="arabic" style={{ fontSize: "clamp(32px, 7vw, 44px)" }}>{opt.arabic}</span>
                </div>
              )}
            </button>
          );
